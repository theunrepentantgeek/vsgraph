...
namespace Niche.Graphs
{
    ...
    public class DotRenderer : IGraphVisitor<IDotStatement>
    {
        ...
        public IDotStatement VisitEdge(Edge edge)
        {
            ...
            var result
                = new DotStatement(text)
                    ...
                    .AddAttribute(Dot.PenWidth, edge.PenWidth.ToString());

            if (edge.Color != Color.Empty)
            {
                result = result.AddAttribute(Dot.Color, ValueOf(edge.Color));
            }
        ...
        public IDotStatement VisitNode(Node node)
        {
            ...
            {
                result = result.AddAttribute(Dot.Color, ValueOf(node.Color));
            }
            ...
            IEnumerable<IDotStatement> subgraphs)
        {
            ...
            var script
                = new DotStatementBlock(keyword + " test ", childStatements)
                    .AddAttribute("smoothType", "avg_dist")
                    .AddAttribute("remincross", "true")
                    .AddAttribute("resolution", graph.DotsPerInch);

            if (graph.MergeEdges)
            {
                script = script.AddAttribute("concentrate", "true");
            }
            ...
            script = script.AddAttribute("rankdir", direction);
